...
def test_employer_template_details_module6():
    ...
    assert 'employer:name' in template_variables(
        'employer'), "Looks like the `employer['name']` is not present in the template."
    assert 'employer:description' in template_variables(
        'employer'), "Looks like the `employer['description']` is not present in the template."


@pytest.mark.test_employer_template_all_jobs_module6
def test_employer_template_all_jobs_module6():
    assert template_exists('employer'), 'The `employer.html` template does not exist in the `templates` folder.'
    assert 'show_jobs:jobs' in template_functions('employer',
                                                  'show_jobs'), 'Have you called the `show_jobs` macro in the `employer.html` file?'
...
def test_employer_template_review_details_module6():
    ...
    assert 'review:title' in template_variables(
        'employer'), "Looks like the `review['title']` is not present in the template."
    assert 'review:status' in template_variables(
        'employer'), "Looks like the `review['status']` is not present in the template."
    assert 'review:date' in template_variables(
        'employer'), "Looks like the `review['date']` is not present in the template."
    assert 'review:review' in template_variables(
        'employer'), "Looks like the `review['review']` is not present in the template."
...
def test_app_employer_route_module6():
    ...
    assert return_values['value/args/s'] == 'employer.html' and return_values[
        'value/func/id'] == 'render_template', 'Did you return the `render_template` call?'
    assert 'employer:employer_id:job:employer_id' in template_functions('_macros',
                                                                        'url_for'), 'Looks like the job title link `href` is incorrect in `_macros.html.'
...
def test_app_employer_route_employers_module6():
    ...
    result = [item for item in get_functions(app.employer) if
              item.startswith('render_template:employer.html:employer:employer')]
...
def test_app_employer_route_jobs_module6():
    ...
    result = [item for item in get_functions(app.employer) if
              item.startswith('render_template:employer.html:employer:employer:jobs:jobs')]
...
def test_app_employer_route_reviews_module6():
    ...
    assert execute_sql in get_functions(app.employer), '`execute_sql` has not been called or has the wrong parameters.'
    result = [item for item in get_functions(app.employer) if
              item.startswith('render_template:employer.html:employer:employer:jobs:jobs:reviews:reviews')]
